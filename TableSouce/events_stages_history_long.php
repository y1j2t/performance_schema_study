<?php
/*
 The events_stages_history_long table contains the N most recent stage events that have ended globally,
 across all threads. Stage events are not added to the table until they have ended. When the table becomes full,
 the oldest row is discarded when a new row is added, regardless of which thread generated either row.

 The Performance Schema autosizes the value of N during server startup. To set the table size explicitly,
 set the performance_schema_events_stages_history_long_size system variable at server startup.
 ...
 events_stages_history_long表包含在所有线程中全局结束的最近的N个阶段事件。阶段事件直到结束后才添加到表中。
 当表满时，当添加新行时，最老的行将被丢弃，而不管哪个线程生成了这两行。

 性能模式会在服务器启动时自动调整N的值。要显式设置表大小，请在服务器启动时设置performance_schema_events_stages_history_long_size系统变量。

 events_stages_history_long表具有与events_stages_current相同的列。参见22.12.5.1节“events_stages_current表”。
 https://dev.mysql.com/doc/refman/5.6/en/performance-schema-events-stages-current-table.html

 events_stages_history_long表允许使用TRUNCATE表。它删除行。
