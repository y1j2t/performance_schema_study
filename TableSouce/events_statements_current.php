<?php
/*
 The events_statements_current table contains current statement events.
 The table stores one row per thread showing the current status of the thread's most recent monitored statement event,
 so there is no system variable for configuring the table size.

 Of the tables that contain statement event rows, events_statements_current is the most fundamental.
 Other tables that contain statement event rows are logically derived from the current events.
 For example, the events_statements_history and events_statements_history_long tables are collections of the most recent statement events that have ended,
 ...
 For information about configuring whether to collect statement events,
 see Section 22.12.6, “Performance Schema Statement Event Tables”.
 
 events_statements_current表包含当前语句事件。该表为每个线程存储一行，显示该线程最近被监视的语句事件的当前状态，因此没有用于配置表大小的系统变量。

 在包含语句事件行的表中，events_statements_current是最基本的。包含语句事件行的其他表从逻辑上派生自当前事件。
 例如，events_statements_history和events_statements_history_long表是最近已经结束的语句事件的集合，每个线程的行数最多，全局分布在所有线程之间。

 有关三个events_statements_xxx事件表之间关系的更多信息，请参见22.9节“当前事件和历史事件的性能模式表”。
 https://dev.mysql.com/doc/refman/5.6/en/performance-schema-event-tables.html

 有关配置是否收集语句事件的信息，请参阅22.12.6节“性能模式语句事件表”。
