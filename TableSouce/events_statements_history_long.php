<?php
/*
The events_statements_history_long table contains the N most recent statement events that have ended globally, across all threads. 
Statement events are not added to the table until they have ended. When the table becomes full, the oldest row is discarded when a new row is added,
...
The events_statements_history_long table has the same columns as events_statements_current. See Section 26.12.6.1, 
“The events_statements_current Table”. Unlike events_statements_current, events_statements_history_long has no indexing.
...
events_statements_history_long表包含在所有线程中全局结束的最近的N个语句事件。语句事件直到结束才添加到表中。当表满时，当添加新行时，最老的行将被丢弃，而不管哪个线程生成了这两行。

N的值在服务器启动时自动调整大小。要显式设置表大小，请在服务器启动时设置performance_schema_events_statements_history_long_size系统变量。

events_statements_history_long表具有与events_statements_current相同的列。参见26.12.6.1节“events_statements_current表”。与events_statements_current不同，events_statements_history_long没有索引。

events_statements_history_long表允许使用TRUNCATE表。它删除行。
