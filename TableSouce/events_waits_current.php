<?php
/*
 The events_waits_current table contains current wait events. The table stores one row per thread showing the current status of the thread's most recent monitored wait event,
 so there is no system variable for configuring the table size.

 Of the tables that contain wait event rows, events_waits_current is the most fundamental. Other tables that contain wait event rows are logically derived from the current events.
 For example, the events_waits_history and events_waits_history_long tables are collections of the most recent wait events that have ended,
 up to a maximum number of rows per thread and globally across all threads, respectively.
 
 events_waits_current表包含当前的等待事件。该表为每个线程存储一行，显示该线程最近被监视的等待事件的当前状态，因此没有用于配置表大小的系统变量。



 在包含等待事件行的表中，events_waits_current是最基本的。包含等待事件行的其他表从逻辑上派生自当前事件。
 例如，events_waits_history和events_waits_history_long表是最近已经结束的等待事件的集合，每个线程的最大行数和所有线程的全局行数分别达到最大值。
*/
...
return [
	...
	'fields' => [
		...
		'EVENT_NAME' => [
			...
			'name' => '工具名集',
			'desc' => '产生该事件的工具的名称。这是来自setup_instruments表的名称值。工具名称可以由多个部分组成，并形成一个层次结构，如26.6节“性能模式乐器命名约定”所讨论的那样。',
