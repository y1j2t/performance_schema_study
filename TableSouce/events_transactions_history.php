<?php
/*
 The events_transactions_history table contains the N most recent transaction events that have ended per thread. Transaction events are not added to the table until they have ended.
 When the table contains the maximum number of rows for a given thread, the oldest thread row is discarded when a new row for that thread is added. When a thread ends, all its rows are discarded.
 
 events_transactions_history表包含每个线程最近结束的N个事务事件。事务事件在结束之前不会添加到表中。当表包含给定线程的最大行数时，当为该线程添加新行时，将丢弃最老的线程行。当一个线程结束时，它的所有行都被丢弃。
*/
...
return [
	...
	'tableName' => 'events_transactions_history',
	'tableDesc' => '每个线程最近的事务事件',
	'fields' => [
		'THREAD_ID' => [
		...
		'EVENT_NAME' => [
			...
			'name' => '工具名称集',
			'desc' => '收集该事件的工具名称。这是来自setup_instruments表的名称值。仪器名称可以由多个部分组成，并形成一个层次结构，如26.6节“性能模式乐器命名约定”所讨论的那样。',
